...
- `PasswordInput` and `SearchInput` keep what was typed or autofilled before
  the page hydrated. Both set the field's value one way, so hydration wrote
  the empty model back over it, and a required password field then stopped
  its form from submitting.
